...
## LinkedIn export privacy repair

`assets/MyLinkedinInfo/` contained private LinkedIn export data and must not be copied into the generated Pages artifact. The source directory is removed in this PR, ignore rules now block recommitting it, Vite uses an allowlist of public asset folders/files instead of copying all of `assets/`, and validation fails if generated output contains private LinkedIn export path/name markers. Previous commits may still contain those exports; purging historical Git objects is a separate security operation that should be planned outside this PR.
